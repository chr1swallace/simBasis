...
createBasisAndProj<-function(DT,basis,proj){
  ...
  ## metrics <- c(#'r_est_maf_se','r_emp_maf_se',
## 'est_shrinkage','emp_shrinkage','ws_est_shrinkage','ws_emp_shrinkage')
  metrics <- c(#'r_emp_maf_se',
'emp_shrinkage',
'ws_emp_shrinkage','shrinkage_nog',
'ws_shrinkage_nog')
  for(m in metrics){
    n <- 'beta'
...
getPairDist <- function(S){
  ...
  tmp.DT[,.(label1=Var1,label2=Var2,d=value)]
...
## make all basis CV BIG
wh <- grep("_",names(scen),invert=TRUE)
for(i in wh)
    scen[[i]]$gammaCV <- 3*scen[[i]]$gammaCV
## if a shared backbone, add a new projected disease with the backbone
if(grepl("backbone",args$scenario_file)) {
    b <- intersect(scen$GWAS1$CV,scen$GWAS2$CV)
    r <- sample(scen$random_2000_2000$CV,6)
    back <- scen[grep("random",names(scen))]
    names(back) %<>% sub("random","back",.)
    for(i in seq_along(back))
        back[[i]]$CV <- c(b,r)
    scen <- c(scen,back)
}
## ## add a large sample size
## big <- scen[grep("5000_5000",names(scen))]
## names(big)  %<>% sub("5000_5000","10000_10000",.)
...
## duplicate projected scenarios so we can measure distances in projected space
dup <- scen[grep("_",names(scen))]
names(dup) <- paste0("R",names(dup))
scen <- c(scen,dup)
...
message("Complete, saving to ",ofile,".RData")

all.pd <- rbindlist(all.pd)
save(all.pd,file=paste0(ofile,".RData")) #"~/Projects/simBasis/100sims-v3.RData")

if(!interactive())
    q("no")

(load("~/Projects/simBasis/100sims-v2.RData"))
...
all.pd[,grp:=paste(mtype,qtype,sep="-")]
all.pd[label1==label2 | label1==paste0("R",label2) | label1==paste0("R",label2),grp:=paste0(grp,"-exact")]
all.pd[qtype==paste0("R",mtype) | mtype==paste0("R",qtype) | mtype==qtype, grp:="similar"]
all.pd[label1==paste0("R",label2) | label1==paste0("R",label2), grp:="exact"]
...
x <- merge(all.pd[label1!=label2 & grepl("shrinkage",metric),],s,by="metric")
x[,size:=paste(sub(".*_","",label1),sub(".*_","",label2),sep="/")]
x[,Z:=(d)/esd] #/esd]

ggplot(x[size=="5000/5000",], aes(x=grp,y=Z,col=gamma)) + geom_violin(draw_quantiles=c(0.25,.5,.75)) + #geom_point() +
  theme(axis.text.x=element_text(angle=90)) + facet_grid(stat~size) + #,scales="free") + 
  background_grid() 
